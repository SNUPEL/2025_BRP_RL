...
class PPO(nn.Module):
    def __init__(self, feature_dim, hidden_dim, lookahead_block_num, grid_size, learning_rate, lmbda, gamma, alpha,
                 beta, epsilon, mod):
        ...
        if mod == 'GAT2':
            self.gnn = GATModel2(self.input_dim, self.hidden_dim, self.U, self.D, self.R, self.L, num_heads=2).to(
                device)
        self.Actor_net = Actor_net(self.hidden_dim + self.lookahead_block_num * feature_dim).to(device)
        self.Critic_net = Critic_net(self.r * self.c * self.hidden_dim + self.lookahead_block_num * feature_dim + 1).to(
            device)
        if mod == 'MLP':
    ...
    def update(self, gridss, blockss, block_leftss, actionss, rewardss, doness, maskss, probss, ep_len, step1,tr_step,
               model_dir):
        ave_loss = 0
        en_loss = 0
        v_loss = 0
        ...
        temp_state_values = self.calculate_v(grids.reshape(-1, r, c, fea), blocks.reshape(b, -1, fea),
                                             block_leftss)  # b,1
        ...
        if step1%20==0:
            ...
            }, model_dir + 'trained_model' + str(tr_step)+'_'+str(step1) + '_.pth')
